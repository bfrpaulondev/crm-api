...
export class UserService {
  ...
  async login(data: LoginData, requestId: string): Promise<AuthResult> {
    return traceServiceOperation('UserService', 'login', async () => {
      ...
      if (data.tenantSlug && data.tenantSlug.length > 0) {
        tenant = Array.from(tenants.values()).find(t => t.slug === data.tenantSlug!.toLowerCase());
      } else if (data.tenantId && data.tenantId.length > 0) {
        tenant = tenants.get(data.tenantId);
      }
      ...
      const user = Array.from(users.values()).find(
        u => u.email === data.email.toLowerCase() && u.tenantId === tenant!._id.toHexString()
      );
